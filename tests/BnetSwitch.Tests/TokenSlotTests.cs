...
public static class TokenSlotTests
{
    ...
    public static void Run()
    {
        ...
        {
            var lastSeen = new Dictionary<string, string> { [SlotA] = TokenStore.Hash(Bytes(1, 2, 3)) };
            var current = new Dictionary<string, byte[]> { [SlotA] = Bytes(1, 2, 3) };
        ...
        {
            var lastSeen = new Dictionary<string, string> { [SlotA] = TokenStore.Hash(Bytes(1, 2, 3)) };
            var current = new Dictionary<string, byte[]> { [SlotA] = Bytes(9, 9, 9) };
        ...
        T.Test("ChangedSince: 新增槽被识别", () =>
        {
            var changed = TokenStore.ChangedSince(
                ...
                new Dictionary<string, byte[]> { [SlotA] = Bytes(1) });
        ...
        T.Test("ChangedSince: 多槽同时变化全部列出(调用方据此放弃学习)", () =>
        {
            ...
            {
                [SlotA] = TokenStore.Hash(Bytes(1)),
                [SlotB] = TokenStore.Hash(Bytes(2)),
            };
            var current = new Dictionary<string, byte[]>
            {
                [SlotA] = Bytes(11),          // 变了
                [SlotB] = Bytes(22),          // 变了
                ["77777777"] = Bytes(33),     // 新增
            };
            var changed = TokenStore.ChangedSince(lastSeen, current);
        ...
        T.Test("ChangedSince: 槽从注册表消失不上报(现行为记录)", () =>
        {
            ...
            var current = new Dictionary<string, byte[]> { [SlotA] = Bytes(1) };
        ...
        T.Test("ChangedSince: 槽名大小写不同按变化处理(现行为记录)", () =>
        {
            ...
            var lastSeen = new Dictionary<string, string> { ["4eb0c645"] = TokenStore.Hash(Bytes(1, 2, 3)) };
            var current = new Dictionary<string, byte[]>(StringComparer.OrdinalIgnoreCase) { ["4EB0C645"] = Bytes(1, 2, 3) };
        ...
        {
            TokenStore.WriteLastSeen(new Dictionary<string, byte[]> { [SlotA] = Bytes(1, 2, 3), [SlotB] = Bytes(4) });
            var seen = TokenStore.ReadLastSeen();
            T.Equal(2, seen.Count, "槽数");
            T.Equal(TokenStore.Hash(Bytes(1, 2, 3)), seen[SlotA], "槽 A 哈希");
            T.Equal(TokenStore.Hash(Bytes(4)), seen[SlotB], "槽 B 哈希");
        ...
        T.Test("学习闭环: 登录一次后恰好识别出该号自己的槽", () =>
        {
            ...
            var before = new Dictionary<string, byte[]> { [SlotA] = Bytes(1), [SlotB] = Bytes(2) };
            TokenStore.WriteLastSeen(before);

            var afterLogin = new Dictionary<string, byte[]> { [SlotA] = Bytes(1), [SlotB] = Bytes(77) };
        ...
        T.Test("注册表边界: 非 Windows 平台安全退化(不读不写不炸)", () =>
        {
            ...
            T.False(store.Write(SlotA, Bytes(1, 2, 3)), "Write 应退化为 false");
        });
    }
}
