...
- **test 现状**：`./build.sh test` 运行 `tests/` 下的回归测试工程（零依赖控制台运行器，任一用例失败即非零退出；「失败→非零」契约本身有用例自检）。「tests/ 为空则打印『无测试』返回 0」只是扩展预留的兜底路径（当前 `tests/BnetSwitch.Tests/` 始终存在），**不会谎称测试通过**。约定见下方「测试约定」。
